...
export class EventService {
    ...
    updateStatus(eventId: number, status: string): Observable<HttpResponse<EventModel>> {
        return this.http.patch<HttpResponse<EventModel>>(
            `${this.apiUrl}/${eventId}/status`,
            status, // Envia só a string crua
            {
                headers: this.getHeaders().set('Content-Type', 'application/json') // importante
            }
        );
    }
